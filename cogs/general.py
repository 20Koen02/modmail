...
class General(commands.Cog):
    ...
    async def stats(self, ctx):
        total_seconds = int(
            (datetime.utcnow() - await self.bot.started()).total_seconds()
        )
        ...
        embed.add_field("Owner", "<@255009837002260482> (original by chamburr)")
        ...
        if ctx.guild:
            embed.add_field(
                "Shards", f"{ctx.guild.shard_id + 1}/{await self.bot.shard_count()}"
            )
        else:
    ...
    # @commands.command(
    #     description="See the amazing stuff we have partnered with.", usage="partners"
    # )
    ...
    #     await ctx.send(
    #         Embed("GitHub Repository", "https://github.com/chamburr/modmail")
    #     )
